...
\newcommand{\everyframe}{100} % use only every nth frame for the movies
	...
	\begin{columns}
		\begin{column}{0.5\linewidth}
			\begin{itemize}
				...
				\item<4->[]
				\item<4-> Zebrafish (\emph{Danio rerio})
			\end{itemize}
		\end{column}
		\begin{column}{0.5\linewidth}
			...
			\includegraphics<4>[width=\imsize]{./img/The_Fry_Cook_Games_085}%
			\only<4>{\source{spongebob.fandom.com/wiki/Huge\_guy}{}}%
		\end{column}
	\end{columns}
\end{frame}
\note{Breathing in insects is just a series of tubes (https://knowyourmeme.com/memes/series-of-tubes)}
\note{For this pilot study, zebrafish were chosen as a well-established model in exercise physiology and in respect to the '3R principle' of animal experimentation.
	As gas exchange and transfer in fish is less efficient than in vertebrates, oxygen uptake might be a bottle-neck in the pathway of oxygen.
	Additionally, the anatomy of the gills allows further growth within the opercular chamber, whereas the plasticity of adult human lung might be limited due to the rigidness of the chest restricting its expansion.
	Our working hypothesis was that exercise would increase oxygen consumption and improve oxygen uptake via gill enlargement.}

\begin{frame}
	\frametitle{Adaptation to training exercise}
	\begin{overprint}
		\begin{itemize}
			\item Increased body size
			\item Skeletal and heart muscle hypertrophy
			\item Induction of angiogenesis in muscles
			\begin{itemize}
				\item \cite{Andersen1977}: \faMale
				\item \cite{Baum2018}: \begin{tikzpicture}
					\def\size{0.618 ex}
			...
			\end{itemize}
			\item<2-> Humans? \onslide<3->{Inconclusive results
			\begin{itemize}
				\item \cite{Baxter-Jones1996}: Athletes \enquote{better} than non-athletes
	...
	\end{overprint}
\end{frame}
\note{Helena adds: \enquote{If the lung function of competitive swimmers (n=10) don't further increase before and after one training season, can we really conclude that the swimmers lung don't adapt? Although there is no solid evidence that adult lung may grow after swimming exercise, IMO there is also not sufficient evidence against: all tests measured lung \emph{function}, not its \emph{morphology}.}}
\note{To summarize, the different parts of the pathway of oxygen show similar training induced adaptation mechanisms in diverse vertebrates including humans, but no data about the morphological response of the respiratory organ to training have been published yet.}
...
\begin{frame}
	...
	\begin{column}{0.5\linewidth}
		\begin{itemize}
			...
			\end{itemize}			
			\item<4-> Scanning electron microscopy
			\begin{itemize}
				\item<4-> Qualitative gill structure
			\end{itemize}
			\item<5-> Critical point drying\onslide<6->{, \uct imaging}\onslide<7->{, delineation in CTAn}\onslide<8->{and analysis}
			\begin{itemize}
				\item<8-> Gill volume, structure and complexity
			\end{itemize}
		\end{itemize}
	\end{column}
	\begin{column}{0.5\linewidth}
		...
		\only<5>{%
			...
			\def\shadow{4}% shadow parameter for scalebar
			\begin{tikzpicture}[x=\imagescale,y=-\imagescale]
				\node[anchor=north west, inner sep=0pt, outer sep=0pt] at (0,0) {\includegraphics[width=\imsize]{./img/{{IMG_20190603_174659}}}};
		...
		\includegraphics<7>[width=\imsize]{./img/CTAn}%
		\only<8>{%
...
\begin{frame}
	...
	\only<2>{\include{./plots/Endurance}}
	\only<3>{\include{./plots/Endurance_scaled}}
\end{frame}
...
\begin{frame}
	...
	\centering
	\only<1>{\Huge\faRuler}
	\only<2>{\include{./plots/Lengths}}
	\only<3>{\include{./plots/Lengths_scaled}}
\end{frame}
...
\note{\emph{scipy.stats.ttest\_ind()}
...
\begin{frame}
	...
	\only<2>{\include{./plots/Weights}}
	\only<3>{\include{./plots/Weights_scaled}}
\end{frame}
...
\begin{frame}
	...
	\only<2>{\include{./plots/Filament_length}}
	\only<3>{\include{./plots/Filament_length_scaled}}
\end{frame}
...
\begin{frame}
	...
	\only<5>{\include{./plots/Gillvolume}}
	\only<6>{\include{./plots/Gillvolume_scaled}}
\end{frame}
...
\begin{frame}
	...
	\only<5>{\include{./plots/Gills_per_organ}}
	\only<6>{\include{./plots/Gills_per_organ_scaled}}
\end{frame}

\begin{frame}
	\frametitle{\ce{O2} consumption}
%	\only<1>{%
	\include{./plots/O2_normalized}
	%}
%	\only<2>{\include{./plots/O2_normalized_scaled}}
\end{frame}
\note{Increase in normalized \ce{O2} consumption relatively to control (because otherwise not significant change)}
...
\begin{frame}
	\frametitle{Pixel size \& Resolution}
	\centering
	\only<1>{%
		\pgfmathsetlength{\imagewidth}{0.5\linewidth}%
		\pgfmathsetlength{\imagescale}{\imagewidth/1487}%
		\begin{tikzpicture}[x=\imagescale,y=-\imagescale]
			\node[anchor=north west, inner sep=0pt, outer sep=0pt] at (0,0) {\includegraphics[width=\imagewidth]{./img/LineProfile}};
			\draw[-,ubRed,ultra thick] (295,537) -- (532,522);
		\end{tikzpicture}%
		}%
	\only<2>{%
		\begin{tikzpicture}
			\begin{axis}[
				...
				xlabel={Pixels},
				ylabel={Gray value}]
				\addplot [ubRed,mark=o] table [col sep=comma] {./plots/LineProfile.csv};
			\end{axis}
		\end{tikzpicture}
		}
\end{frame}
