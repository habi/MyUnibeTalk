...
% generally thicker lines by default
...
\pgfkeys{%
	/pgfplots/tuftelike/.style={%
		tick style={major tick length=4pt},
		separate axis lines,
		axis x line*=bottom,
...
\newcommand{\everyframe}{50} % use only every nth frame for the movies
...
\defbeamertemplate{footline}{unibe}{%
	\hspace*{\fill}%
	v. \href{https://github.com/habi/20190605_BrukerUserMeeting/commit/\gitHash}{\gitAbbrevHash}\xspace|%
...
\begin{frame}[label=current]
	...
	\begin{overprint}
			...
			\begin{itemize}
				...
				\item \faFish: \cite{Sanger1992}
			\end{itemize}
			\item Increase in hemoglobin levels
			\item<2-> Human lungs? \onslide<3->{Inconclusive results
			\begin{itemize}
				\item Athletes better than non-athletes: \cite{Baxter-Jones1996}
				\item Competitive swimming does not affect lung growth: \cite{Bovard2018}
%			\note{Helena adds: \quote{If the lung function of competitive swimmers (n=10) don't further increase before and after one training saison, can we really conclude that the swimmers lung don't adapt? Although there is no solid evidence that adult lung may grow after swimming exercise, IMO there is also not sufficient evidence against: all tests measured lung \emph{function}, not its \emph{morphology}}
				\item Forced expiratory volume and maximum voluntary ventilation higher in swimmers than runners \cite{Sable2012,Armour1993}
				\item Other indices of lung function similar between swimmers, runners and control \cite{Armour1993}
			\end{itemize}}
...
\begin{frame}
	...
	\begin{column}{0.5\linewidth}
		\begin{itemize}
			\item<1-> Training according to \textcite{Palstra2010} (Training for \SI{5}{w}, \SI{5}{\day\per w}, \SI{6}{\hour\per\day}, at $\frac{2}{3}$ of critical speed).
			\note{Comparable with the training routine of Michael Phelps weekly training routine but the fish trained only for ca. \SI{7}{\percent} of their lifetime and started as adults while the Olympic winner for perhaps half of his lifetime and started as child)}
	...
	\begin{column}{0.5\linewidth}
		...
		\includegraphics<7>[height=0.6\textheight]{./img/CTAn}%
		\only<8>{%
...
\begin{frame}
	...
	\only<1>{%
		...
		\def\x{2019}% scalebar-x starting at golden ratio of image width of 2524px = 1560
...
\begin{frame}
	...
	\only<4>{\include{./img/Gills_per_organ}}
\end{frame}

\begin{frame}
	\frametitle{\ce{O2} consumption}
	\include{./img/O2_normalized}
	\note{Increase in normalized o2 consumption relatively to control (because otherwise not significant change)}
\end{frame}
	...
	\begin{columns}
		...
		\begin{itemize}
			...
			\begin{itemize}
				...
				\item<1-> Fluri Wieland, Oleksiy-Zakhar Khoma
		...
		\begin{column}{0.382\linewidth}
			\includegraphics<1->[width=\imsize]{./img/team}
		\end{column}
	\end{columns}
\end{frame}
