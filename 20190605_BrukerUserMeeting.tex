...
\newcommand{\everyframe}{55} % use only every nth frame for the movies
...
\begin{frame}
	...
	\begin{overprint}
			...
			\begin{itemize}
				\item \textcite{Andersen1977}: \faMale
				\item \textcite{Baum2018}: \begin{tikzpicture}
					\def\size{0.618 ex}
					\filldraw (0,0) circle (\size);
					\filldraw (\size,\size) circle (0.618*\size);
					\filldraw (-\size,\size) circle (0.618*\size);
					\end{tikzpicture}: 
				\item \textcite{Sanger1992}: \faFish
			\end{itemize}
			\item Increase in hemoglobin levels
			\item<2-> Human lungs? \onslide<3->{Inconclusive results
			\begin{itemize}
				\item \textcite{Baxter-Jones1996}: Athletes better than non-athletes
				\item \textcite{Bovard2018}: Competitive swimming does not affect lung growth: 
				...
				\item \textcite{Sable2012,Armour1993}: Forced expiratory volume and maximum voluntary ventilation higher in swimmers than runners
				\item \textcite{Armour1993}: Other indices of lung function similar between swimmers, runners and control 
			\end{itemize}}
...
\begin{frame}
	...
	\begin{tikzpicture}[remember picture,overlay]%
		\node[xshift=0,yshift=-0.0625\paperheight-0.016\paperheight/2] at (current page.center){%
			\animategraphics[autoplay,width=\paperwidth,every=\everyframe]{25}{./mov/fishhead/head-without-gills0}{001}{160}%
			};%
	\end{tikzpicture}%
\end{frame}
...
\begin{frame}
	...
	\begin{column}{0.5\linewidth}
		\begin{itemize}
			\item<1-> Training according to \textcite{Palstra2010}\newline\SI{5}{w}, \SI{5}{\day\per w}, \SI{6}{\hour\per\day}, at \SI{66}{\percent} of critical speed
...
\begin{frame}[label=current]
	...
	\only<1>{\faRuler}
	\only<2>{\include{./img/Lengths}}
	\only<3>{\include{./img/Lengths_scaled}}
...
\begin{frame}
	...
	\only<1>{\faWeight}
	\only<2>{\include{./img/Weights}}
	\only<3>{\include{./img/Weights_scaled}}
...
\begin{frame}
	...
	\only<1>{Tachometer}
	\only<2>{\include{./img/Endurance}}
	\only<3>{\include{./img/Endurance_scaled}}	
\end{frame}
...
\begin{frame}
	...
	\only<2>{\include{./img/Filament_length}}
	\only<3>{\include{./img/Filament_length_scaled}}	
\end{frame}
...
\begin{frame}
	...
	\only<6>{\include{./img/Gillvolume_scaled}}
\end{frame}
...
\begin{frame}
	...
	\only<5>{\include{./img/Gills_per_organ}}
	\only<6>{\include{./img/Gills_per_organ_scaled}}
\end{frame}

\begin{frame}
	\frametitle{\ce{O2} consumption}
	\only<1>{\include{./img/O2_normalized}}
	\only<2>{\include{./img/O2_normalized_scaled}}
